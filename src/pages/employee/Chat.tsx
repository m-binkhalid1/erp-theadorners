...
const EmployeeChat = () => {
  ...
  const extractEvent = async (messageId: string, content: string) => {
    ...
    try {
      ...
      if (resp.status === 429) { toast.error("AI thoda busy hai, baad mein try karein"); return; }
      if (resp.status === 402) { toast.error("AI credits khatam ho gaye"); return; }
      if (!resp.ok) throw new Error("AI processing failed");

      const result = await resp.json();
      if (result.is_event) {
        toast.success("🎉 AI ne event detect kar liya!", { description: `Company: ${result.extracted.company || "Unknown"}` });
      }
    } catch (err) {
  ...
  const deleteMessage = async (msgId: string) => {
    ...
    const { error } = await supabase.from("chat_messages").delete().eq("id", msgId).eq("sender_id", user?.id || ""); // Extra safety for employee
    if (error) toast.error(error.message);
  };
  ...
  const bulkDelete = async () => {
    ...
    const { error } = await supabase.from("chat_messages")
      ...
      .eq("sender_id", user?.id || ""); // Employees can only delete their own
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2 min-h-0">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-lg text-muted-foreground font-medium">Koi message nahi hai</p>
            <p className="text-sm text-muted-foreground/60">Neeche se message bhejein</p>
          </div>
        )}
        {messages.map((msg) => {
          ...
          return (
            ...
            >
              {selectionMode && !isDeleted && isMine && (
                <button 
              ...
              {selectionMode && !isMine && <div className="w-7 shrink-0" />}
              ...
              <div 
                className={cn(
                  ...
                  (selectionMode && isMine && !isDeleted) ? "cursor-pointer" : ""
                )}
                onClick={() => {
                  if (selectionMode && isMine && !isDeleted) {
              ...
              >
                {/* Delete Button - Employee can only delete their own message */}
                {isMine && !selectionMode && !isDeleted && (
                  <button
                ...
                {/* Sender Name */}
                {!isMine && !isDeleted && (
                  <p className="text-[11px] font-bold text-primary mb-1">{profiles[msg.sender_id] ?? "User"}</p>
                )}
